...
\begin{equation}\label{d4:eq:bone-counts}
 ...
 \end{cases}
\end{equation}
Indeed, the invariant fixes the number of right stones, and the area fixes
the total number of trihexes.  Finally, summing~\eqref{d4:eq:energy-table} over
the tiling gives
\[
 E_{a,d}=B+3(s+f).
\]
...
For $d=3$, Theorem~\ref{d4:thm:defects} gives zero defects.  For $d=4$, it gives
exactly one.  We now classify and count that one-defect case.
...
unless it is a source or terminal.  At the three defect owners, the bad tile
covers the label-$\ell$ cell and the first label-$\ell$ edge covers the other
two cells.  At a one-cell boundary owner, an incoming edge covers its unique
cell, so that owner can only be terminal.
The label potentials in~\eqref{d4:eq:potential} strictly change along directed
edges, so directed cycles are impossible.  Summing indegree minus outdegree
over each component, and using the three defect incidences and the three
boundary deficits, rules out an internal three-incoming terminal and pairs
each defect owner with one boundary owner.  Local cell labels are preserved
through a pass-through owner; hence the endpoints have the same label.
\end{proof}
...
in-phase right stone at every unused full owner.  At a core owner, the bad
cell and first edge cover all present cells; at a pass-through owner, the
incoming edge covers the path label and the outgoing edge covers the other
two; and at the boundary endpoint, the incoming edge covers the unique cell.
Thus the construction is a literal exact cover.  Its local description also
shows that it inverts deletion of the bad tile.
