...
Both enumerative chains and the general finite-defect theorem have kernel-only
Lean~4 formalizations over Mathlib.}
...
The Problem 6 kernel-only Lean~4 formalization defines the literal benzel and
prototiles, proves the tiling/path equivalence, and verifies the generating
function and closed form.  Its exact 19-label map compiles with
\texttt{--trust=0}.  The separate $d=4$ development proves the general
finite-defect theorem, identifies its $d=4$ literal carrier definitionally,
and kernel checks the one-defect bijection, ballot sum, and Lagrange--Good
evaluation.  The final audits in both tracks report only the standard Lean
axioms \texttt{propext}, \texttt{Classical.choice}, and \texttt{Quot.sound}.
Finite computations remain consistency checks rather than formal premises.
...
The combined source and $d=4$ formalization will accompany the next versioned
release of the reproducibility archive.
