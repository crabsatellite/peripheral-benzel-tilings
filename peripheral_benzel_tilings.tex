...
\hypersetup{
  ...
  pdftitle={Finite-Defect Path Models for Peripheral Benzel Tilings: Propp's Problem 6 and the First Defect Diagonal},
...
\title[Finite-defect peripheral benzel tilings]
{Finite-Defect Path Models for Peripheral Benzel Tilings:\\
Propp's Problem 6 and the First Defect Diagonal}
...
$\Bz(n,2n-3)$ by right stones and all three orientations of bones.  We first
prove his conjectured formula
\[
 T_{103}(n,2n-3)
 =\frac{(3n+3)(3n-7)!}{(n-5)!(2n-1)!}\qquad(n\geq5).
\]
The proof partitions the cells into periodic three-cell owners.  A linear
owner-label energy turns every tiling into a directed $Y$ of three labelled
ballot paths, whose two sink chiralities are evaluated by a three-variable
constant term and Lagrange inversion.  We then place this rigid case in a
finite-defect hierarchy on the diagonals $b=2a-d$.  In the nonoverlap range
$2k\le a-1$, the exact defect number is $\binom{k}{2}$ for $d=3k$ and
$\binom{k+1}{2}$ for $d=3k+1$.
Consequently $d=4$ is the first one-defect diagonal.  Deleting its unique
defect gives three independent labelled ballot paths and five defect classes.
A literal inverse construction and a specialized Lagrange--Good evaluation
yield an algebraic ordinary generating function for every
$T_{103}(a,2a-4)$.
\end{abstract}
...
common endpoint has three incoming edges with distinct predecessor owners,
so their last directions are distinct.  The permitted directions therefore
use $A,B,C$ once each.  Nonnegativity of the predecessor coordinates for
the incoming $C,B,A$ edges forces, respectively, $u,v,w>0$ at the sink.
Any further component would have no sink and hence, by
...
An accompanying Lean~4 development formalizes both enumerative arguments and
the general finite-defect theorem directly over the literal benzel and
exact-cover definitions.  It includes the tiling/path equivalences, the
one-defect classification, the ballot sums, and both generating-function
evaluations.  The checked public endpoints use no project-specific axioms or
computational proof shortcuts; their reported logical dependencies are only
the standard Lean/Mathlib principles \texttt{propext},
\texttt{Classical.choice}, and \texttt{Quot.sound}.  The finite computations
remain consistency checks rather than premises of the proofs.

\paragraph{Preprint history.}
Earlier versions under concept DOI
\href{https://doi.org/10.5281/zenodo.22057378}
{\nolinkurl{10.5281/zenodo.22057378}} contained only the solution of
Propp's Problem~6.  The present article subsumes that version and adds the
finite-defect hierarchy and the complete first-defect diagonal.

\paragraph{Data and code availability.}
The released Problem 6 formalization and reproducibility files are available in the
\href{https://github.com/crabsatellite/peripheral-benzel-tilings}{public code
repository}; the versioned software archive has DOI
\href{https://doi.org/10.5281/zenodo.22079396}{10.5281/zenodo.22079396}.
The combined source and $d=4$ formalization are included in the versioned
releases of the same repository.
